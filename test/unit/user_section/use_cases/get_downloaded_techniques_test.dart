...
@injectable.LazySingleton(as: ITechniqueRepository, env: [GET_DOWNLOADED_TECHNIQUES_TEST_ENV])
class TechniqueRepositoryMock extends Mock implements ITechniqueRepository {}

void main() {
  testConfigureDependencies(GET_DOWNLOADED_TECHNIQUES_TEST_ENV);
  ...
  test("[GetDownloadedTechniques] should return a list of downloaded techniques when invoked", () async {
    ...
    final getDownloadedTechniques = serviceLocator<GetDownloadedTechniques>();
    final techniqueRepositoryMock = serviceLocator<ITechniqueRepository>();
    when(() => techniqueRepositoryMock.getDownloadedTechniques()).thenAnswer((_) async => fetchedTechniques);

    final res = await getDownloadedTechniques();
