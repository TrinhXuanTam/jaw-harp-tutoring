...
@injectable.LazySingleton(as: ITechniqueRepository, env: [GET_MOST_RECENT_TECHNIQUES_TEST_ENV])
class TechniqueRepositoryMock extends Mock implements ITechniqueRepository {}

void main() {
  testConfigureDependencies(GET_MOST_RECENT_TECHNIQUES_TEST_ENV);
  ...
  test("[GetMostRecentTechniques] should return a list of most recent techniques when invoked", () async {
    ...
    final getMostRecentTechniques = serviceLocator<GetMostRecentTechniques>();
    final techniqueRepositoryMock = serviceLocator<ITechniqueRepository>();
    when(() => techniqueRepositoryMock.getMostRecentTechniques()).thenAnswer((_) async => fetchedTechniques);

    final res = await getMostRecentTechniques();
