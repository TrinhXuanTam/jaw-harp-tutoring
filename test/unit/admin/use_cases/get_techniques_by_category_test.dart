...
@injectable.LazySingleton(as: ITechniqueAdminRepository, env: [GET_TECHNIQUES_BY_CATEGORY_TEST_ENV])
class TechniqueAdminRepository extends Mock implements ITechniqueAdminRepository {}

void main() {
  testConfigureDependencies(GET_TECHNIQUES_BY_CATEGORY_TEST_ENV);
  ...
  test("[GetTechniquesByCategory] should return list of all techniques of given category when invoked.", () async {
    final techniqueAdminRepository = serviceLocator<ITechniqueAdminRepository>();
    final getTechniquesByCategory = serviceLocator<GetTechniquesByCategory>();
    when(() => techniqueAdminRepository.getTechniquesByCategory(category)).thenAnswer((_) async => [technique]);
