// Auth
const String SPLASH_SCREEN_ROUTE = "/splash";
const String HOME_SCREEN_ROUTE = "/home";
const String AUTH_SCREEN_ROUTE = "/authentication";
const String PASSWORD_RESET_SCREEN_ROUTE = "/passwordReset";
const String PASSWORD_RESET_SENT_SCREEN_ROUTE = "/passwordResetSent";
const String SIGN_UP_SCREEN_ROUTE = "/signUp";
const String LINK_AUTH_PROVIDERS_SCREEN_ROUTE = "/linkAuthProviders";
const String EMAIL_VERIFICATION_UP_SCREEN_ROUTE = "/emailVerification";

// Admin
const String ADMIN_MENU_SCREEN_ROUTE = "/admin";
const String MANAGE_CATEGORIES_SCREEN_ROUTE = "/manageCategories";
const String CREATE_CATEGORY_SCREEN_ROUTE = "/createCategory";
